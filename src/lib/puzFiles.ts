import { PuzFileCallbacks } from "@entities/PuzFileCallbacks";
...
export async function processPuzData(data: Blob, callbacks?: PuzFileCallbacks): Promise<Puzzle | undefined> {
    ...
    let puzAuthor = "";
    let puzTitle = "";
    [puzTitle, i] = getNextString(restOfFile, i);
    puzzle.title = (callbacks && callbacks.titleFunc) ? callbacks.titleFunc(puzTitle) : puzTitle;
    [puzAuthor, i] = getNextString(restOfFile, i);
    [puzzle.copyright, i] = getNextString(restOfFile, i);
    puzzle.authors = (callbacks && callbacks.authorFunc) ? callbacks.authorFunc(puzAuthor) : [puzAuthor];
