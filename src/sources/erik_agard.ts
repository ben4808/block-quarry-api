...
export async function scrapeErikAgard(endDate: Date, startUrl?: string): Promise<Puzzle[]> {
    let puzzles = [] as Puzzle[];
    
    let html: any;
    let filepath = `C:\\Users\\ben_z\\Documents\\bq_puzzles\\Erik_Agard\\agard_fullpage.html`;
    ...
    for (let post of posts) {
        ...
        for (let i = 0; i < puzLinks.length; i++) {
            ...
            if (!puzzle) continue;
            puzzle.date = date;
            puzzle.publication = "Glutton For Pun";
            ...
            puzzle.storedPuzLink = `C:\\Users\\ben_z\\Documents\\bq_puzzles\\Erik_Agard\\${downloadFilename}`;
